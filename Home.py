...
def build_message_list(user_query):
    ...
    zipped_messages = [SystemMessage(
        content= """You are a Health Care Expert for ICNA Relief, here to guide and assist people with their health questions and concerns. Please provide accurate and helpful information, and always maintain a polite and professional tone.
                    ...
                    Make sure your response should be in the same language in which you will get the text.
                    ...
                    Your focus should be on answer and generate your response based on this.
...
# for file in ["images/english_text.png", 
#              "images/urdu_text.png",
#              "images/spanish_text.png",
#              "images/bangla_text.png",
#              "images/arabic_text.png",]:
#     with open(file, "rb") as image:
#         encoded = base64.b64encode(image.read()).decode()
#         images.append(f"data:image/jpeg;base64,{encoded}")
...
with st.sidebar:
    ...
    if clicked > -1:
        ...
        # st.warning("Choose the language first. By default it is English")
        selected_value = 'English'
        
    selected_language = language_mapping[selected_value]
    st.session_state['demographic'] = selected_value
    st.session_state['medication'] = selected_value
    st.session_state['health'] = selected_value
    st.session_state['relation'] = selected_value
    st.session_state['feeling'] = selected_value
    st.session_state['treatment'] = selected_value
    st.session_state['addiction'] = selected_value
    
    # if st.button(language_content[selected_language]['button_text'], key='one'):
    #     file_path = r"D:\OSTF\StreamlitCloud\Intake_form_filled.html"
    #     webbrowser.open("file://" + file_path, new=2)
        
    if st.button(language_content[selected_language]['button_text'], key='two'):
        file_content = open("Intake_form_filled.html").read()
        st.download_button(label="Download HTML File", data=file_content, file_name="Intake_form_filled.html", mime="text/html")
