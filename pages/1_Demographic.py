...
def dialogue1(selected_language):
    ...
    if st.session_state.dialogue_1 <= 1:
        st.info(f"Q{questions[st.session_state.question_number - 1][1]}- {questions[st.session_state.question_number - 1][language_index]}")
        speak(questions, st.session_state.dialogue_1, selected_language)
        audio_bytes = audio_recorder(key=f"Q{st.session_state.dialogue_1}", 
                                     icon_size="2x",
...
with st.sidebar:
    ...
    if clicked > -1:
        ...
        selected_value = st.session_state.get('demographic')
