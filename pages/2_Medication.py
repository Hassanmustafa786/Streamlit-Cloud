...
def dialogue2(selected_language):
    ...
    if st.session_state.dialogue_2 <= 2:
        st.info(f"Q{questions[st.session_state.question_number - 7][1]}- {questions[st.session_state.question_number - 7][language_index]}")
        speak(questions, st.session_state.dialogue_2, selected_language)
        audio_bytes = audio_recorder(key=f"Q{st.session_state.dialogue_2}", 
                                     icon_size="2x",
...
with st.sidebar:
    ...
    if clicked > -1:
        ...
        selected_value = st.session_state.get('medication')
