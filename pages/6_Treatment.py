...
def dialogue6(selected_language):
    ...
    if st.session_state.dialogue_6 <= 6:
        st.info(f"Q{questions[st.session_state.question_number - 3][1]}- {questions[st.session_state.question_number - 3][language_index]}")
        speak(questions, st.session_state.dialogue_6, selected_language)
        audio_bytes = audio_recorder(key=f"Q{st.session_state.dialogue_6}", 
                                     icon_size="2x",
...
with st.sidebar:
    ...
    if clicked > -1:
        ...
        selected_value = st.session_state.get('treatment')
